...
static void check_for_completed_rows(void) {	
	uint8_t i;
	uint8_t j;
		// check for a completed row, all ones by looping over the array 
		for (i=0; i<=(BOARD_ROWS-1); i++){
			if (board[i] == ((1<<BOARD_WIDTH)-1)){
				// add to score 
				add_to_score(100);
				add_to_row(1);
				update_seven_seg(get_row());
				// zero out the array 
				board[i]=0;
				// loop over the array to move the rows down and reflect cleared rows on the LED MATRIX
				for(j=i; j>0; j--){
					board[j]=board[j-1];
					copy_matrix_column(board_display[j-1], board_display[j]);
				}
				// Call function again to double check for any simultaneous complete rows  
				//check_for_completed_rows();
			i--;
			}
			else{
		...
		set_matrix_column_to_colour(board_display[0] , COLOUR_BLACK);
